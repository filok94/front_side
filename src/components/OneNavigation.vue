...
// default borders
const stringifiedBordersOfMainCircle = ref('76px 53px 45px 92px')
const stringifiedBordersOfActiveTarget = ref('63px 78% 92% 45%')
// change borders
...
const logoutTabRef: HTMLRef = ref(null)
const wrapperForCircle = ref(null)
  ...
  if (!isCirclesActive.value && navCircle.value?.offsetHeight && navCircle.value?.offsetWidth) {
    ...
    circles.value.forEach((e, index) => {
      if (e != null && navCircle.value) {
        switch (index) {
          case 0: {
            animateFrom(e, 'fromTop', ANIMATIONS_RANGE.LOW)
            break
          }
          case 1: {
          ...
          case 2: {
            animateFrom(e, 'fromRight', ANIMATIONS_RANGE.LOW)
            break
          }
          case 3: {
          ...
          case 4: {
            animateFrom(e, 'fromBottom', ANIMATIONS_RANGE.LOW)
            break
          }
          case 5: {
          ...
          case 6: {
            animateFrom(e, 'fromLeft', ANIMATIONS_RANGE.LOW)
            break
          }
        }
      }
...
const emit = defineEmits<{(e: 'activation', target: number): void }>()
...
const button = (event: Event) => {
  const indexOfElem = Array.from(
...
const signOutFromNavCircle = () => signOut()
